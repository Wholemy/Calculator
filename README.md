...
Калькулятор имеет множество полей ввода и различный функционал, поля A B R M Y X дробные числа, с целым и делителем в отдельных полях, они используются кнопками сверху калькулятора для всех представленных операций, кнопки показывают и скрывают поля, не затрагивая их значений) Так же есть упрощенный режим D где все выражение добавлется в одной строке сверху и результат выводится в строки ниже, сразу после изменения верхней строки) Поля можно добавлять и копировать кнопкой над полем D++, скрывать показывать кнопками Dx и убавлять кнопкой D-- начиная с поледнего поля)

Отличительная особенность прямого режима, числа со знаком минус '-' создаются при условии что знак перед первым числом в выражении или в скобках, так как знаки все знаки между числами управляют вычислениями двух чисел и их количество не ограничено) Таким образом можно 2***2, возводя первое число в степень второго, ***2 будет умножением самого себя на себя 2 * 2 = 4 * 4 = 16 * 16 = 256)
